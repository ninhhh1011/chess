...
export default function Learn() {
  ...
  if (selected) return <section>
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_440px]">
      <article className="rounded-xl border border-slate-800 bg-slate-800 p-8 ">
        <h1 className="text-4xl font-bold">{selected.title}</h1>
        <p className="mt-6 text-lg leading-9 text-slate-300">{selected.content}</p>
        <h2 className="mt-8 text-2xl font-extrabold">Ví dụ minh họa</h2>
        <p className="mt-3 leading-8 text-slate-300">{selected.example}</p>
      ...
      <div className="mx-auto w-full max-w-[440px] rounded-xl border border-slate-800 bg-slate-800 p-4 shadow-sm ">
        <Chessboard options={{
  ...
  return <section>
    <h1 className="text-4xl font-bold md:text-5xl">Học cờ</h1>
    <p className="mt-4 max-w-3xl text-slate-300">Chọn một bài học ngắn, dễ hiểu để làm quen từng luật quan trọng của cờ vua.</p>
