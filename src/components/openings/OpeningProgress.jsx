export default function OpeningProgress({ progress }) {
  ...
  return <div>
    ...
    {progress?.attempts ? <p className="mt-2 text-xs text-slate-100/50">{progress.attempts} lần luyện · {progress.successCount} lần hoàn thành</p> : <p className="mt-2 text-xs text-slate-400">Chưa có tiến độ.</p>}
  </div>;
}
