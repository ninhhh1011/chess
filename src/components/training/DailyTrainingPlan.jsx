export default function DailyTrainingPlan({ plan, onRegenerate, onStartLesson, onStartExercises, onStartOpening }) {
  return <article className="rounded-xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/15 via-white/[.07] to-white/[.04] p-6 shadow-sm ">
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-4">
      <div className="rounded-xl bg-slate-900 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-400">{plan?.lesson?.reason || 'Chưa đủ dữ liệu nên ôn lại bài cũ.'}</p>
      </div>
      <div className="rounded-xl bg-slate-900 p-5">
      ...
      <div className="rounded-xl bg-slate-900 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-400">{plan?.opening?.reason || 'Luyện 3-5 nước đầu để xây nền khai cuộc.'}</p>
      </div>
      <div className="rounded-xl bg-slate-900 p-5">
        <p className="font-bold text-emerald-500">4. Chơi</p>
        <p className="mt-2 text-sm leading-6 text-slate-300">{plan?.challenge || 'Chơi 1 ván và review lại.'}</p>
      </div>
    </div>
